...
$post_validate = new Validation();
$post_validation = $post_validate->check($_POST, array(
   'fromdate' => array(
       'required' => true,
       ),
   'todate' => array(
...
$get_validate = new Validation();
$get_validation = $get_validate->check($_GET, array(
   'type' => array(
       'required' => true,
       )
    ));
...
if(!$get_validate->passed()){
    Redirect::error($get_validate->errors());
}
if(!$post_validate->passed()){
    Redirect::error($post_validate->errors());
}
...
if ($type == "ExportConf" || $sections == "all") {
    $query = "(SELECT c.id, c.confdate AS date, DATE_FORMAT(c.confdate, '%b %Y') " 
            . "AS confdate, c.title, c.location, c.days, r.attended, r.id AS req_id, "
            . "0 AS editable, 1 AS confirmed, 1 AS verified "
            . "FROM conferences c, requests r WHERE r.userid=? AND r.conf_id=c.id "
            . "AND r.attended = 1 AND c.confdate BETWEEN ? AND ?) "
            . "UNION"
            . "(SELECT id AS id, date, DATE_FORMAT(date, '%b %Y') AS confdate, "
            . "title, location, days, 0 AS attended, 0 AS req_id, "
            . "1 AS editable, confirmed, verified "
            . "FROM suppconfrecords WHERE userid=? "
            . "AND (confirmed IS NULL || confirmed = 1) AND date BETWEEN ? AND ?) "
            . "ORDER BY date DESC";
    ...
    date("j M Y", strtotime($fromdate)) . " and " . date("j M Y", 
            strtotime($todate)) . ":</p>";
    echo "<p>";
...
if ($type == "ExportPub" || $sections == "all") {
    ...
    date("Y", strtotime($fromdate)) . " and " . date("Y", strtotime($todate)) 
            . ":</p>";
    echo "<p>";
    echo "<table style=\"width: 100%;\">
            <thead>
